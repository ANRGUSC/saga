...
import numpy as np
from saga.general_deprecated.ScheduleType.schedule_type import ScheduleType
import networkx as nx

from saga.scheduler import Task


def selector(network: nx.Graph,
        ranking: List[Hashable],
        scheduler : ScheduleType,
        selection_metric: Callable[[str, str, Dict, Dict]]) -> Dict[Hashable, List[Task]]:
    ...
    for task_name in ranking:
        ...
        for node in network.nodes:  # Find the best node to run the task
            finish_time = get_earliest_finish(node, task_name, comp_schedule, task_schedule)

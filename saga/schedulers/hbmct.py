...
def calculate_est(
    ...
    """
    est_table = {
        task_name: {node: None for node in network.nodes} for task_name in group
    }
    for task_name in group:
        for node in comp_schedule:
            ...
            if comp_schedule[node]:
                est_table[task_name][node] = max(
                    max_arrival_time, comp_schedule[node][-1].end
                )
            else:
...
def get_initial_assignments(
    ...
    for task_name in group:  # Assign nodes based on execution times
        ...
        assigned_node = min(
            runtimes, key=lambda node, task_name=task_name: runtimes[node][task_name]
        )
...
def get_ft_after_insert(
    ...
    new_assignments.append(
        new_task_name
    )  # Todo: O(n) time insertion instead of sorting
...
def delete_task_from_schedule(
    ...
    for j in range(i, len(new_schedule)):
        ...
        new_schedule[j] = Task(
            node, task.name, start_time, start_time + runtimes[node][task.name]
        )

    new_ft = 0
    logging.debug(
        "Schedule of %s after remoiving %s : %s", node, task_name, new_schedule
    )
    if new_schedule:
...
class HbmctScheduler(Scheduler):
    ...
    Link: https://ieeexplore.ieee.org/abstract/document/1303065
    """
    ...
    def schedule_groups(
        ...
        for group in groups:
            comp_group_start_positions = {
                node: len(comp_schedule[node]) for node in comp_schedule
            }
            ...
            # print("EST:",est_table)
            average_est = {
                task_name: np.mean([est for est in est_table[task_name]])
            ...
            for node in assignments:
                for task_name in assignments[node]:
                    ...
                    task = Task(
                        ...
                        task_name,
                        start_time,
                        start_time + runtimes[node][task_name],
                    )
            ...
            while assignment_changed:
                ...
                avg_est_assignments = sorted(
                    ...
                    key=lambda task_name, average_est=average_est: average_est[
                        task_name
                    ],
                )
                ...
                for task_name in avg_est_assignments:
                    ...
                    if (
                        new_max_ft < max_ft
                    ):  # Check if removing the task actually improves the max_ft for the node
                        min_ft_node = None

...
def heft_schedule(duplicate_factor: int = 1):
    network, task_graph = get_instance()
    scheduler = HeftScheduler(duplicate_factor=duplicate_factor)
...
def main():
    ...
    heft_sched_dup = heft_schedule(duplicate_factor=2)
    draw_schedule(heft_sched_dup, 'heft_gantt_dup')
    bf_sched = brute_force_schedule()
    draw_schedule(bf_sched, 'brute_force_gantt')

    max_makespan = max(
        task.end 
        for sched in [schedule, heft_sched, bf_sched, heft_sched_dup]
    ...
    draw_schedule(heft_sched_dup, 'heft_gantt_dup_scaled', xmax=max_makespan)

...
_POLICY_LABEL = {
    ...
    "checkpoint_quarterly": "Checkpoint-Quarterly",
...
def _throughput_ratios(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for tp_name in tp_schedulers:
        ...
        merged["SchedulerName"] = f"TP-{algo}-{_POLICY_LABEL[policy]}"
...
def _reschedule_count_tr_by_ccr() -> pd.DataFrame:
    ...
    tp_rows = tp_rows[tp_rows["Variant"] == "TP"]
...
def reschedule_count_table() -> Path:
    ...
    caption = "Table: Reschedule Rate and TP-Static Throughput Improvement statistics."
...
def _improvement_rows(metric: str = "Throughput") -> pd.DataFrame:
    ...
    merged["Variant"] = merged["Algo"].str.endswith("-Tp").map({True: "TP", False: "Base"})
    return merged


def _maxtp_rows(metric: str = "Throughput") -> pd.DataFrame:
    """Same shape as _improvement_rows, but for MaxTP only (Variant "TP" -
    ...
    belong; callers that want MaxTP pooled into "TP" (e.g. the WfCommons TP
...
def wfcommons_tp_ccr_improvement_table() -> Path:
    ...
    caption = "Table: WfCommons TP Reschedule Policy Improvement over Static, by CCR."
...
_CCR_OVERVIEW_VARIANT_ALGOS = {"Base": ["HEFT", "CPoP"], "TP": ["HEFT-Tp", "CPoP-Tp", "MaxTP"]}
...
def reschedule_improvement_plot() -> Path:
    ...
    out_path = reschedule_improvement_plot_dir / "reschedule_improvement_plot.png"
...
_TWO_SERIES_ORDER = ["Base-Static", "TP-Reschedule"]
_TWO_SERIES_COLOR = {"Base-Static": "#2a78d6", "TP-Reschedule": "#eb6834"}
_TP_TWO_SERIES_ORDER = ["TP-Static", "TP-Reschedule"]
_TP_TWO_SERIES_COLOR = {"TP-Static": "#2a78d6", "TP-Reschedule": "#eb6834"}
...
def _draw_two_series_boxplot(
    ...
    (Base-Static vs TP-Reschedule); pass overrides to plot a different pair."""
...
def _two_series_boxplots(
    ...
    TP-Reschedule, y-axis ThroughputRatio (Throughput / best-on-that-instance,
    ...
    for algo in ["HEFT", "CPoP"]:
        for branch in _BRANCH_LABEL:
            ...
            rows = _two_series_ratio_rows(branch, first_name, first_label, second_name, "TP-Reschedule")
            if rows.empty:
                continue
...
def tp_static_vs_tp_reschedule_boxplots() -> list:
    ...
    TP-Static (the throughput-comparator base with no policy) instead of
    Base-Static (the plain EFT-comparator base) against TP-Reschedule."""
    return _two_series_boxplots(
        tp_static_vs_tp_reschedule_dir, "tp_static_vs_tp_reschedule", "{algo}-Tp_static", "TP-Static",
...
_ALL_BASELINES_ORDER = [
    "Base-Static", "TP-Static", "TP-Reschedule", "FastestNode", "MaxTP-Static", "MaxTP-Reschedule",
]
_ALL_BASELINES_COLOR = {
    "Base-Static": "#2a78d6",       # blue
    "TP-Static": "#0089a3",         # teal
    "TP-Reschedule": "#eb6834",     # orange
    "FastestNode": "#4a3aa7",       # violet
    "MaxTP-Static": "#008300",      # green
    "MaxTP-Reschedule": "#e34948",  # red
}
...
def all_baselines_vs_best_boxplots() -> list:
    ...
    added alongside Base-Static and TP-Reschedule: TP-Static (the
    throughput-comparator base with no policy), the two standalone
    heuristics FastestNode and MaxTP-Static (MaxTP's own bare scheduler
    name - see _maxtp_rows), and MaxTP-Reschedule - the latter three are
    ...
    for algo in ["HEFT", "CPoP"]:
        for branch in _BRANCH_LABEL:
            scheduler_labels = {
                ...
                f"{algo}-Tp_static": "TP-Static",
                f"{algo}-Tp_reschedule": "TP-Reschedule",
                "FastestNode": "FastestNode",
                "MaxTP": "MaxTP-Static",
                "MaxTP_reschedule": "MaxTP-Reschedule",
            }
            ...
            out_path = all_baselines_vs_best_dir / f"{name}.png"
            fig.savefig(out_path, dpi=150)
            plt.close(fig)
            print(f"{name} -> {out_path}")
            paths.append(out_path)
...
def _all_policy_boxplot(branch: str, algos: list, title: str, out_name: str) -> Path:
    ...
    return _save_boxplot(
        ...
        out_path=all_policy_boxplots_dir / f"{out_name}.png",
...
def _all_policy_boxplots_with_workflows(branch: str, algos: list, title_prefix: str, out_prefix: str) -> list:
    ...
    paths = [_save_boxplot(
        ...
        out_path=all_policy_boxplots_dir / f"{out_prefix}.png",
        policy_order=_ALL_POLICY_ORDER, policy_color=_ALL_POLICY_COLOR,
    )]
    for workflow in sorted(rows["Workflow"].unique()):
        ...
        paths.append(_save_boxplot(
            ...
            out_path=all_policy_boxplots_dir / f"{out_prefix}_{workflow}.png",
            policy_order=_ALL_POLICY_ORDER, policy_color=_ALL_POLICY_COLOR,
        ))
    return paths
    ...
    for branch in _BRANCH_LABEL:
        ...
        for variant in _BOXPLOT_VARIANTS:
            ...
            paths.append(_save_boxplot(
                ...
                out_path=outdir / f"{name_prefix}_{branch}_{variant.lower()}.png",
            ))
            for workflow in sorted(subset["Workflow"].unique()):
                ...
                paths.append(_save_boxplot(
                    ...
                    out_path=outdir / f"{name_prefix}_{branch}_{variant.lower()}_{workflow}.png",
                ))
    return paths
...
def reschedule_relative_boxplots() -> list:
    ...
    paths = [_save_boxplot(
        ...
        out_path=relative_boxplots_dir / "reschedule_relative_boxplot_wfcommons_tp.png",
    )]
    for workflow in sorted(rows["Workflow"].unique()):
        wf_subset = rows[rows["Workflow"] == workflow]
        paths.append(_save_boxplot(
            wf_subset, title=f"WfCommons (TP) - relative to Reschedule - {workflow}", y_label=y_label,
            out_path=relative_boxplots_dir / f"reschedule_relative_boxplot_wfcommons_tp_{workflow}.png",
        ))
    return paths
...
def reschedule_highlight_boxplots() -> list:
    ...
    for algo, workflow in _HIGHLIGHT_CASES:
        ...
        paths.append(_save_boxplot(
            subset, title=f"TP-{base_algo} - {workflow.capitalize()}", y_label="Throughput Ratio",
            out_path=highlight_boxplots_dir / f"reschedule_boxplot_{base_algo.lower()}_tp_{workflow}.png",
        ))
    return paths

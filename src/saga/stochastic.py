...
from saga import (
    Schedule, ScheduledTask, TaskGraph, Network,
    NetworkNode, NetworkEdge,
    TaskGraphNode, TaskGraphEdge
)
...
class StochasticNetwork(BaseModel):
    ...
    nodes: FrozenSet[StochasticNetworkNode] = Field(..., description="The nodes in the network.")
    edges: FrozenSet[StochasticNetworkEdge] = Field(..., description="The edges in the network.")
    ...
    def create(cls,
               ...
               edges: Iterable[StochasticNetworkEdge | Tuple[str, str, RandomVariable | float] | Tuple[str, str]]) -> 'StochasticNetwork':
        ...
        for n in nodes:
            if isinstance(n, StochasticNetworkNode):
                ...
                node_speeds[n[0]] = n[1] if isinstance(n[1], RandomVariable) else RandomVariable(samples=[n[1]])
            else:
                raise ValueError(f"Invalid node: {n}")

        edge_dict: Dict[Tuple[str, str], RandomVariable] = {}
        for e in edges:
            src, dst = sorted((e.source, e.target)) if isinstance(e, StochasticNetworkEdge) else sorted((e[0], e[1]))
            default_speed: RandomVariable = RandomVariable(samples=[0.0]) if src != dst else RandomVariable(samples=[math.inf])
            speed: RandomVariable = (
                e.speed if isinstance(e, StochasticNetworkEdge) else
                (e[2] if isinstance(e[2], RandomVariable) else RandomVariable(samples=[e[2]]))
                if isinstance(e, tuple) and len(e) == 3 else
                default_speed
            )
        ...
        for n1, n2 in product(node_speeds.keys(), repeat=2):
            ...
            if (src, dst) not in edge_dict:
                default_speed = RandomVariable(samples=[0.0]) if src != dst else RandomVariable(samples=[math.inf])
    ...
    def from_nx(cls,
                ...
                edge_speed_attr: str = "weight") -> 'StochasticNetwork':
        ...
        edges = [
            StochasticNetworkEdge(source=u, target=v, speed=G.edges[u, v][edge_speed_attr])
            for u, v in G.edges
        ]
    ...
    def get_edge(self,
                 source: str | StochasticNetworkNode,
                 target: str | StochasticNetworkNode) -> StochasticNetworkEdge:
...
class StochasticTaskGraphNode(BaseModel):
    ...
    cost: RandomVariable = Field(..., description="The cost (computation time) of the task.")
    ...
    def sample(self) -> TaskGraphNode:
        ...
        return TaskGraphNode(
            name=self.name,
            cost=self.cost.sample(num_samples=1)[0]
        )
...
class StochasticTaskGraph(BaseModel):
    ...
    tasks: FrozenSet[StochasticTaskGraphNode] = Field(..., description="The tasks in the task graph.")
    dependencies: FrozenSet[StochasticTaskGraphEdge] = Field(..., description="The dependencies in the task graph.")
    ...
    def create(cls,
               ...
               dependencies: Iterable[StochasticTaskGraphEdge | Tuple[str, str, RandomVariable | float] | Tuple[str, str]]) -> 'StochasticTaskGraph':
        ...
        for t in tasks:
            if isinstance(t, StochasticTaskGraphNode):
                ...
                task_cost = t[1] if isinstance(t[1], RandomVariable) else RandomVariable(samples=[t[1]])
        ...
        for d in dependencies:
            if isinstance(d, StochasticTaskGraphEdge):
                ...
                size = d[2] if isinstance(d[2], RandomVariable) else RandomVariable(samples=[d[2]])
                dependency_set.add(StochasticTaskGraphEdge(source=d[0], target=d[1], size=size))
            elif isinstance(d, tuple) and len(d) == 2:
                dependency_set.add(StochasticTaskGraphEdge(source=d[0], target=d[1], size=RandomVariable(samples=[0.0])))
            else:
        ...
        if len(sources) != 1:
            ...
            super_source = StochasticTaskGraphNode(name="__super_source__", cost=RandomVariable(samples=[0.0]))
            task_set.add(super_source)
            for source in sources:
                dependency_set.add(StochasticTaskGraphEdge(source=super_source.name, target=source.name, size=RandomVariable(samples=[0.0])))
        if len(sinks) != 1:
            logging.warning("Task graph has multiple sinks; adding super sink.")
            super_sink = StochasticTaskGraphNode(name="__super_sink__", cost=RandomVariable(samples=[0.0]))
            task_set.add(super_sink)
            for sink in sinks:
                dependency_set.add(StochasticTaskGraphEdge(source=sink.name, target=super_sink.name, size=RandomVariable(samples=[0.0])))
    ...
    def from_nx(cls,
                ...
                edge_weight_attr: str = "weight") -> 'StochasticTaskGraph':
        """Create a StochasticTaskGraph from a NetworkX directed graph."""
        tasks = [StochasticTaskGraphNode(name=n, cost=G.nodes[n][node_weight_attr]) for n in G.nodes]
        dependencies = [StochasticTaskGraphEdge(source=u, target=v, size=G.edges[u, v][edge_weight_attr]) for u, v in G.edges]
    ...
    def all_topological_sorts(self) -> Generator[List[StochasticTaskGraphNode], None, None]:
    ...
    def in_edges(self, task: str | StochasticTaskGraphNode) -> List[StochasticTaskGraphEdge]:
        """Get the incoming edges of a task."""
        task = task.name if isinstance(task, StochasticTaskGraphNode) else task
        edges = []
        for src, tgt in self.graph.in_edges(task):
            edge_data = self.graph.get_edge_data(src, tgt)
            edges.append(StochasticTaskGraphEdge(source=src, target=tgt, size=edge_data['weight']))
        return edges
    ...
    def out_edges(self, task: str | StochasticTaskGraphNode) -> List[StochasticTaskGraphEdge]:
    ...
    def get_dependency(self, source: str | StochasticTaskGraphNode, target: str | StochasticTaskGraphNode) -> StochasticTaskGraphEdge:
        ...
        return StochasticTaskGraphEdge(source=source, target=target, size=edge['weight'])
...
class StochasticScheduledTask(BaseModel):
    ...
    name: str = Field(..., description="The name of the task.")
    rank: float = Field(..., description="The rank of the task, determining the order of execution on the node.")
...
class StochasticSchedule(BaseModel):
    ...
    task_graph: StochasticTaskGraph = Field(..., description="The task graph being scheduled.")
    network: StochasticNetwork = Field(..., description="The network the tasks are scheduled on.")
    mapping: Dict[str, List[StochasticScheduledTask]] = Field(..., description="The mapping of tasks to nodes.")
    ...
    def __init__(self,
                 ...
                 mapping: Optional[Dict[str, List[StochasticScheduledTask]]] = None) -> None:
        super().__init__(
    ...
    def makespan(self) -> RandomVariable:
        ...
        return RandomVariable(
            samples=[
                self.sample().makespan for _ in range(DEFAULT_NUM_SAMPLES)
            ]
        )
    ...
    def sample(self) -> Schedule:
        ...
        while not pq.empty():
            ...
            start_time = schedule.get_earliest_start_time(det_task, det_node, append_only=True)
            schedule.add_task(
                task=ScheduledTask(
                    node=task.node,
                    name=task.name,
            ...
            for out_edge in det_task_graph.out_edges(task.name):
                if all(schedule.is_scheduled(in_edge.source) for in_edge in det_task_graph.in_edges(out_edge.target)):
                    succ_task = next(t for t in self.mapping[task.node] if t.name == out_edge.target)
                    pq.put((-succ_task.rank, succ_task))
        return schedule

    def __getitem__(self, node: str | StochasticNetworkNode) -> List[StochasticScheduledTask]:
    ...
    def remove_task(self, task_name: str | StochasticTaskGraphNode) -> None:
        ...
        task_name = task_name.name if isinstance(task_name, StochasticTaskGraphNode) else task_name

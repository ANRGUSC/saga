        ...
        for processor in network.nodes:
            ...
            for edge in child_edges:
                ...
                for next_processor in network.nodes:
                    if processor.name == next_processor.name:
                        pass_time = 0.0
                    else:
                        pass_time = (
                            task_graph.get_dependency(task.name, child_name).size
                            / network.get_edge(
                                processor.name, next_processor.name
                            ).speed
                        )
                    ...
                    candidate_cost = (
                        pass_time
                        + next_task_time
                        + table[child_name][next_processor.name]
                    )
...
def peft_rank_sort(
    task_graph: TaskGraph, oct_table: Dict[str, Dict[str, float]]
    ...
    """
    rank_oct = {
        task: sum(proc_map.values()) / len(proc_map) if proc_map else 0.0
        for task, proc_map in oct_table.items()
    }
    rev_topo_sort = {
    ...
    order = sorted(
        rank_oct.keys(),
        key=lambda x: (rank_oct[x], rev_topo_sort.get(x, -1)),
        reverse=True,
    )
    ...
    def schedule(
        ...
        while ready_list:
            ...
            for child_edges in task_graph.out_edges(task_name):
                ...
                if all(
                    schedule.is_scheduled(edge.source) for edge in parent_edges
                ) and (
                    not schedule.is_scheduled(child_name)
                    and (child_name not in ready_list)
                ):

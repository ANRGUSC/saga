...
class TempHeftScheduler(ScheduleInjector, Scheduler):
    ...
    def _do_schedule( #ADD OPTIONAL CALLS
            self,
            network: nx.Graph,
        ...
        remaining = [t for t in task_graph.nodes if t not in task_map]
        subgraph  = task_graph.subgraph(remaining).copy()
        schedule_order = heft_rank_sort(network, subgraph)
                ...
                max_arrival_time: float = max(  #
                    [
                        ...
                        *[
                            task_map[parent].end
                            + (
                                commtimes[(task_map[parent].node, node)][
                                    (parent, task_name)
                                ]
                            )
    ...
    #     runtimes, commtimes = TempHeftScheduler.get_runtimes(network, task_graph)

...
class NPFLBScheduler(DWScheduler):
    ...
    """
    def schedule(self, network: Graph, task_graphs: List[Tuple[nx.DiGraph, float]]) -> Dict[Hashable, List[Task]]:
        ...
        for task_graph_tupple in task_graphs:
            ...
            for task in task_graph.nodes:
                if task_graph.in_degree(task) == 0:
                    non_ep_tasks.put((task_graph_arrival_time, task))

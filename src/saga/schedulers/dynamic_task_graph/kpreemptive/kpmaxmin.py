...
class KPMaxMinScheduler(DWScheduler): # pylint: disable=too-few-public-methods
    """Max-Min scheduler"""

    def __init__(self, k: int = 1) -> None:
        super().__init__()
        self.k = k
        self.name = f"{self.k}PMaxMinScheduler"
    ...
    def schedule(self,
        ...
        for i in range(len(task_graphs)):
            task_graph = nx.compose_all([task_graphs[j][0] for j in range(max(0, i-k+1), i+1)])

...
def avg_TEC(task_name: str, network: Network, task_graph: TaskGraph) -> float:
    task = task_graph.get_task(task_name)
    return sum(task.cost / node.speed for node in network.nodes) / len(list(network.nodes))
...
def calc_TL(task_name: str, network: Network, task_graph: TaskGraph, assigned_tasks: Dict[str, str]) -> float:
    ...
    for in_edge in in_edges:
        ...
        if assigned_tasks.get(pred) is None or assigned_tasks.get(task_name) is None:
            ...
            network_edge = network.get_edge(assigned_tasks[pred], assigned_tasks[task_name])
...
def calc_BL(task_name: str, network: Network, task_graph: TaskGraph, assigned_tasks: Dict[str, str]) -> float:
    ...
    for out_edge in out_edges:
        ...
        if assigned_tasks.get(succ) is None or assigned_tasks.get(task_name) is None:
            ...
            network_edge = network.get_edge(assigned_tasks[task_name], assigned_tasks[succ])
...
def calc_priority(task_name: str, network: Network, task_graph: TaskGraph, assigned_tasks: Dict[str, str]) -> float:
    return calc_BL(task_name, network, task_graph, assigned_tasks) - calc_TL(task_name, network, task_graph, assigned_tasks)
...
class DPSScheduler(Scheduler):
    ...
    @staticmethod
    def get_runtimes(network: Network, task_graph: TaskGraph) -> Tuple[Dict[str, Dict[str, float]], Dict[Tuple[str, str], Dict[Tuple[str, str], float]]]:
        ...
        for node in network.nodes:
            ...
            for task in task_graph.tasks:
                ...
                logging.debug(f"Task {task.name} on node {node.name} has runtime {runtimes[node.name][task.name]}")
        ...
        for edge in network.edges:
            ...
            for dep in task_graph.dependencies:
                ...
                logging.debug(f"Task {dep.source} on node {src} to task {dep.target} on node {dst} has communication time {commtimes[src, dst][dep.source, dep.target]}")
    ...
    def _schedule(self,
        ...
        if schedule is not None:
            ...
            task_schedule = {task.name: task for _, tasks in schedule.items() for task in tasks}
        ...
        while len(ready_list) > 0:
            ...
            best_node = next(iter(network.nodes)).name, 0  # arbitrary initialization
            for node in network.nodes:
                        ...
                        [
                            min_start_time, *[
                                task_schedule[in_edge.source].end + (
                                    commtimes[(task_schedule[in_edge.source].node, node.name)][(in_edge.source, task_name)]
                                )
                                for in_edge in in_edges
                            ]
                        ]
                ...
                idx, start_time = get_insert_loc(comp_schedule[node.name], max_arrival_time, runtime)

                finish_time = start_time + runtime
                if finish_time < min_finish_time:
                    min_finish_time = finish_time
                    best_node = node.name, idx
            ...
            assigned_tasks[task_name] = best_node[0]

...
def upward_rank(network: Network, task_graph: TaskGraph) -> Dict[str, float]:
    ...
    for task in topological_order[::-1]:
        ...
        max_comm_time = 0 if task_graph.out_degree(task.name) <= 0 else max(
            [
                ranks[task_graph_dependency.target] + np.mean([
...
def downward_rank(network: Network, task_graph: TaskGraph) -> Dict[str, float]:
    ...
    for task in task_graph.topological_sort():
        rank = 0 if task_graph.in_degree(task.name) <= 0 else max(
            [
                ranks[task_graph_dependency.source] + np.mean([
                    task_graph_dependency.size / network_edge.speed
                    for network_edge in network.edges
                ]) + (task.cost / np.mean([
                    neighbor.speed
...
class CpopScheduler(Scheduler):
    ...
    def schedule(self,
        ...
        if schedule is not None:
            ...
            task_map = {task.name: task for _, tasks in schedule.items() for task in tasks}
        ...
        while pq:
            ...
            ready_tasks = [
                task_graph.get_task(dep.target) for dep in task_graph.out_edges(task.name)
                if all(child_dep.source in task_map for child_dep in task_graph.in_edges(dep.target))
            ]

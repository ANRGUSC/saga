...
def compare_distributions(dist1_samples, dist2_samples, atol=0.01, rtol=0.1):
    ...
    assert np.isclose(mean1, mean2, atol=atol, rtol=rtol), (
        f"Mean mismatch: {mean1} vs {mean2}"
    )
    assert np.isclose(var1, var2, atol=atol, rtol=rtol), (
        f"Variance mismatch: {var1} vs {var2}"
    )
